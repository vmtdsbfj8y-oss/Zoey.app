...
export function BureauTabs({
  ...
  return (
    <View
      ...
      style={{ backgroundColor: 'rgba(44,24,84,0.42)', borderWidth: 1, borderColor: 'rgba(168,85,247,0.18)' }}>
      {BUREAU_ORDER.map((bureau) => {
        ...
        return (
          <Pressable
            ...
            style={active ? { backgroundColor: tokens.violet500 } : undefined}>
            <Text
              className="font-sans-semibold text-[12.5px]"
              style={{ color: active ? tokens.parchment : has ? 'rgba(244,239,255,0.62)' : 'rgba(244,239,255,0.3)' }}>
              {bureau}
            </Text>
            {/* A dot, not a number: presence at a glance without implying a value. */}
            <View
              className="mt-1 h-1 w-1 rounded-full"
              style={{ backgroundColor: has ? (active ? tokens.parchment : tokens.violet400) : 'transparent' }}
            />
          </Pressable>
        );
      })}
