...
export default function ResetPasswordScreen() {
  ...
  return <ScreenBackground idPrefix="reset"><SafeAreaView className="flex-1 justify-center px-6"><View className="rounded-[28px] border border-white/10 bg-ink-900/80 p-5"><Text className="font-display text-[24px] text-parchment">{t('auth.newPasswordTitle')}</Text><TextInput editable={ready} secureTextEntry value={password} onChangeText={setPassword} placeholder={ready ? 'New password' : 'Opening secure reset link…'} placeholderTextColor="#777187" className="mt-5 rounded-[18px] border border-white/10 bg-ink-800 px-4 py-4 text-parchment" /><Pressable disabled={!ready} onPress={update} className="mt-5 items-center rounded-full bg-violet-600 py-4"><Text className="text-white">{t('auth.updatePassword')}</Text></Pressable></View></SafeAreaView></ScreenBackground>;
}
